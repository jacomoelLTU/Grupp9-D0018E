...
function getImage($conn, $postId): string{
    ...
    while($row=mysqli_fetch_array($result, MYSQLI_ASSOC)){
        ...
        if (!empty($row['post_img'])) {
          ...
          return "style='background-image:url(data:image/jpeg;base64, '../pictures/No-image-Placeholder.png');";
        }
    }
    return "";
}
...
while($row=mysqli_fetch_array($query, MYSQLI_ASSOC)){
    ...
    <div id='postItem' " .getImage($conn, $row['post_id'])."'> 
        <a class='showItemText' href='../pages/showPost.php?postId=".$row['post_id']."&postTitle=".$row['post_title']."&postDescription=".$row['post_description']."'></div>
